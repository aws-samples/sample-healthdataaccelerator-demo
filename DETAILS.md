...
The Patient 360 Dashboard includes an optional generative-AI feature (patient
record summary and Q&A) built on Amazon Bedrock. It is a **demonstration** and
is **not** clinical decision support: it does not diagnose, recommend treatment,
or interpret results. The feature ships with responsible-AI controls including
"facts only" prompting, an Amazon Bedrock Guardrail (denied medical-advice
topic, content filters, and prompt-attack protection), human oversight, and
least-privilege access. See [RESPONSIBLE_AI.md](./RESPONSIBLE_AI.md) for the
full description, configuration, and limitations. Before any production or
regulated use, perform your own risk assessment and validation.
...
```
"security_group_ip_range_ipv4": "203.0.113.10/32",
```

Which will give access to only `203.0.113.10` (replace with your own public IP).
 ...
 * `security_group_ip_range_ipv4`       Set to a [IPV4 cidr](https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing#IPv4_CIDR_blocks) to allow access to a group of IPV4 addresses. Use the narrowest range possible (for example your corporate egress `203.0.113.10/32`). **Do not use `0.0.0.0/0`** — that exposes the service to the entire internet and is inappropriate for a system handling PHI. Defaults to "null" which allows no access to any IPV4 addresses.
 * `security_group_ip_range_ipv6`       Set to a [IPV6 cidr](https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing#IPv6_CIDR_blocks) to allow access to a group of IPV6 addresses. Use the narrowest range possible. **Do not use `::/0`** — that exposes the service to the entire internet. Defaults to "null" which allows no access to any IPV6 addresses.
...
> **Responsible AI notice:** The clinical example above is for demonstration only. Generative AI outputs can be inaccurate or incomplete and must **not** be used as the sole basis for any clinical, diagnostic, or treatment decision. Any AI-assisted output should be reviewed and validated by a qualified healthcare professional (human-in-the-loop) before use. When applying this integration to real Protected Health Information (PHI), you are responsible for HIPAA compliance, executing an AWS Business Associate Addendum (BAA), configuring [Amazon Bedrock Guardrails](https://docs.aws.amazon.com/bedrock/latest/userguide/guardrails.html), and adding appropriate disclaimers for end users. This sample ships with synthetic data only.
