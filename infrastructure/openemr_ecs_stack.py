...
"""
CDK Stack for OpenEMR on ECS.

PHI / HIPAA NOTICE:
OpenEMR is an electronic health record (EHR) system that stores and processes
protected health information (PHI) - patient demographics, clinical notes,
medications, and diagnoses. If you process real PHI, this is a HIPAA-regulated
...
from infrastructure.security_infrastructure import SecurityInfrastructure
import hashlib
import string
...
class OpenemrEcsStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        self.emr_serverless_release_label = "emr-7.0.0"
        ...
        self.lambda_python_runtime = _lambda.Runtime.PYTHON_3_12

        # Use pinned production image. 4GB memory to prevent OOM kills.
        self.openemr_version = "8.0.0-2026-06-21"
        ...
        # Creates shared security infrastructure (KMS keys and VPC endpoints)
        self.security_infrastructure = SecurityInfrastructure(
            self,
        ...
        # Creates automatic data loader that populates OpenEMR with synthetic patient data after deployment
        # This provides a "batteries included" demo experience
        self._create_automatic_data_loader()
    ...
    def _create_backup(self):
        ...
        # Temporarily disabled to avoid backup vault conflicts during initial deployment
    ...
    def _create_alb(self):
        ...
        self.alb = elb.ApplicationLoadBalancer(
            ...
            idle_timeout=Duration.seconds(300)  # Increased for slow FHIR queries (MedicationRequest, Immunization)
        )
    ...
    def _create_db_instance(self):
        ...
        if self.node.try_get_context("enable_bedrock_integration") == "true":
            # ------------------------------------------------------------------
            # RESPONSIBLE AI / HUMAN OVERSIGHT NOTICE (Aurora ML -> Bedrock):
            # This optional integration lets Aurora invoke Amazon Bedrock
            # foundation models from within an EHR (PHI) workload. Generative
            # output must NEVER be treated as clinical decision support:
            #   - Keep a qualified human clinician in the loop for any output
            #     that informs care; the model produces documentation aids only.
            #   - Model access is scoped to specific, named foundation models in
            #     this stack's region (no wildcard) via the
            #     `aurora_ml_bedrock_model_ids` context value.
            #   - A Bedrock Guardrail (denied medical-advice topic + content and
            #     prompt-attack filters) is provisioned when
            #     `enable_bedrock_guardrails` is true; the application layer must
            #     pass this guardrail identifier on every InvokeModel call.
            # See RESPONSIBLE_AI.md and DETAILS.md for the full requirements
            # (BAA, HIPAA eligibility, human-in-the-loop) before enabling this
            # on real PHI.
            # ------------------------------------------------------------------

            # Scope model access to explicitly configured foundation models in
            # this region only, instead of a wildcard across all models/regions.
            _model_ids_ctx = (
                self.node.try_get_context("aurora_ml_bedrock_model_ids")
                or "amazon.nova-lite-v1:0"
            )
            bedrock_model_ids = [
                m.strip() for m in _model_ids_ctx.split(",") if m.strip()
            ]
            bedrock_model_arns = [
            ...
            # Responsible-AI guardrail for the Aurora ML -> Bedrock path. Mirrors
            # the Patient 360 dashboard guardrail: blocks medical advice and
            # harmful content and mitigates prompt injection.
            self.aurora_ml_guardrail = None
            self.aurora_ml_guardrail_version = None
            if self.node.try_get_context("enable_bedrock_guardrails") != "false":
                self.aurora_ml_guardrail = bedrock.CfnGuardrail(
                    self,
                    "AuroraMlBedrockGuardrail",
                    name="openemr-aurora-ml-guardrail",
                    description=(
                        "Responsible-AI guardrail for the OpenEMR Aurora ML -> "
                        "Bedrock integration. Blocks medical advice and harmful "
                        "content; mitigates prompt injection."
                    ),
                    blocked_input_messaging=(
                        "This request can't be processed. This integration does "
                        "not provide medical advice."
                    ),
                    blocked_outputs_messaging=(
                        "The response was blocked. This integration reports "
                        "documented facts only and cannot provide medical "
                        "advice, diagnoses, or treatment guidance. Please "
                    ...
                    topic_policy_config=bedrock.CfnGuardrail.TopicPolicyConfigProperty(
                        topics_config=[
                            bedrock.CfnGuardrail.TopicConfigProperty(
                                ...
                                type="DENY",
                                definition=(
                                    "Providing medical advice, diagnoses, "
                                    "prognoses, treatment plans, medication "
                                    "guidance, or any clinical interpretation "
                                    "or recommendation."
                                ),
                                examples=[
                ...
                self.aurora_ml_guardrail_version = bedrock.CfnGuardrailVersion(
                    self,
                    "AuroraMlBedrockGuardrailVersion",
                    guardrail_identifier=self.aurora_ml_guardrail.attr_guardrail_id,
            ...
            database_ml_role.add_to_policy(
                iam.PolicyStatement(
                    ...
                    # Scoped to the specific foundation model ARN(s) in this
                    # region rather than a wildcard across all models/regions.
                    resources=bedrock_model_arns,
                )
            )
            # Allow (and require) the guardrail to be applied at inference time,
            # scoped to this stack's guardrail resource only.
            if self.aurora_ml_guardrail:
                database_ml_role.add_to_policy(
                    iam.PolicyStatement(
                        actions=['bedrock:ApplyGuardrail'],
                        resources=[self.aurora_ml_guardrail.attr_guardrail_arn],
                    )
                )
                parameters["aws_bedrock_guardrail_id"] = self.aurora_ml_guardrail.attr_guardrail_id
                parameters["aws_bedrock_guardrail_version"] = self.aurora_ml_guardrail_version.attr_version
        ...
        # Get min capacity from context, default to 0.5 to prevent cold starts
        db_min_capacity = self.node.try_get_context("aurora_serverless_min_capacity") or 0.5
        ...
        if self.node.try_get_context("enable_data_api") == "true":
            self.db_instance = rds.DatabaseCluster(self, "DatabaseCluster",
                                                   ...
                                                   serverless_v2_min_capacity=db_min_capacity,
                                                   serverless_v2_max_capacity=256,
                                                   storage_encrypted=True,
                                                   storage_encryption_key=self.security_infrastructure.data_encryption_key,
        ...
        if self.node.try_get_context("enable_bedrock_integration") == "true":
            ...
            bedrock_runtime_interface_endpoint.add_to_policy(
                iam.PolicyStatement(
                    ...
                    actions=['bedrock:InvokeModel', 'bedrock:InvokeModelWithResponseStream'],
                    # Scoped to the specific foundation model ARN(s) in this
                    # region (same set granted to the Aurora ML role above)
                    # rather than a wildcard across all models/regions.
                    resources=bedrock_model_arns,
                    effect=iam.Effect.ALLOW,
                )
            )
    ...
    def _create_efs_volume(self):
        ...
        # Create EFS for sites folder with customer-managed KMS key
        self.file_system_for_sites_folder = efs.FileSystem(
            self,
            "EfsFileSystemForSitesFolder",
            vpc=self.vpc,
            encrypted=True,
            kms_key=self.security_infrastructure.data_encryption_key,
            removal_policy=RemovalPolicy.DESTROY,
        )
        ...
        # Create EFS for ssl folder with customer-managed KMS key
    ...
    def _create_and_maintain_tls_materials(self):
        ...
        # RunTask is scoped to the specific SSL-materials task definition
        # (plus the cluster condition), not a wildcard resource.
        run_task_statement = iam.PolicyStatement(
            effect=iam.Effect.ALLOW,
            actions=["ecs:RunTask"],
            resources=[create_ssl_materials_task.task_definition_arn],
        )
        run_task_statement.add_condition("ArnEquals", {"ecs:cluster": self.ecs_cluster.cluster_arn})

        # DescribeTasks targets task run ARNs that are only known at runtime, so
        # it uses a resource wildcard constrained to this cluster via condition.
        describe_tasks_statement = iam.PolicyStatement(
            effect=iam.Effect.ALLOW,
            actions=["ecs:DescribeTasks"],
            resources=["*"],
        )
        describe_tasks_statement.add_condition("ArnEquals", {"ecs:cluster": self.ecs_cluster.cluster_arn})

        # Add permissions to task role
        create_ssl_materials_task.grant_run(create_ssl_materials_lambda.grant_principal)
        create_ssl_materials_lambda.add_to_role_policy(run_task_statement)
        create_ssl_materials_lambda.add_to_role_policy(describe_tasks_statement)
        ...
        # Add permissions to task role (scoped RunTask + DescribeTasks, same
        # as the scheduled SSL-materials Lambda above).
        create_ssl_materials_task.grant_run(self.one_time_create_ssl_materials_lambda.grant_principal)
        self.one_time_create_ssl_materials_lambda.add_to_role_policy(run_task_statement)
        self.one_time_create_ssl_materials_lambda.add_to_role_policy(describe_tasks_statement)
    ...
    def _create_openemr_service(self):
        ...
        # This script sets up certificates and runs OpenEMR setup.
        # IMPORTANT: The Redis TLS fix must run AFTER openemr.sh because openemr.sh
        # hardcodes REDIS_PATH="tcp://${REDIS_SERVER}:6379" and overwrites any earlier config.
        # Solution: Run openemr.sh in background, wait for Redis config to be created, then fix it.
        # PHP max_execution_time increased to 300s for slow FHIR API queries (MedicationRequest, Immunization)
        command_array = [
            'CA_CERT=$([ -f /swarm-pieces/ssl/certs/ca-certificates.crt ] && echo /swarm-pieces/ssl/certs/ca-certificates.crt || echo /etc/ssl/certs/ca-certificates.crt) && \
            OPENEMR_SH=$([ -f /var/www/localhost/htdocs/openemr/openemr.sh ] && echo /var/www/localhost/htdocs/openemr/openemr.sh || ([ -f /var/www/localhost/htdocs/openemr.sh ] && echo /var/www/localhost/htdocs/openemr.sh || echo /var/www/html/openemr.sh)) && \
            curl --cacert $CA_CERT -o /root/certs/redis/redis-ca \
            --create-dirs https://www.amazontrust.com/repository/AmazonRootCA1.pem && \
            chown apache /root/certs/redis/redis-ca && \
            curl --cacert $CA_CERT -o /root/certs/mysql/server/mysql-ca \
            ...
            sed -i "s|tcp://\\${REDIS_SERVER}|tls://\\${REDIS_SERVER}|g" $OPENEMR_SH && \
            sed -i "s/max_execution_time = 60/max_execution_time = 300/g" /etc/php*/php.ini && \
            sed -i "s/max_input_time = 60/max_input_time = 300/g" /etc/php*/php.ini && \
            $OPENEMR_SH'
        ]
        ...
        openemr_container = openemr_fargate_task_definition.add_container("OpenEMRContainer",
                                                                          ...
                                                                          entry_point=["/bin/sh", "-c"],
                                                                          command=command_array,
                                                                          # Health check is a loopback to the container's own
                                                                          # self-signed TLS endpoint on localhost. -k is required
                                                                          # here (self-signed cert) and safe: there is no network
                                                                          # path or MITM surface for a localhost health probe.
                                                                          health_check=ecs.HealthCheck(
                                                                              command=["CMD-SHELL",
                                                                                       "curl -f -k https://localhost:443/swagger || exit 1"],
        ...
        self.openemr_service = openemr_application_load_balanced_fargate_service.service
        ...
        cfn_openemr_service = self.openemr_service.node.default_child
        ...
        # Least-privilege CloudWatch Logs for the execution role: create and
        # write log groups/streams scoped to this account/region, instead of
        # the broad CloudWatchLogsFullAccess managed policy.
        openemr_fargate_task_definition.execution_role.add_to_policy(
            iam.PolicyStatement(
                actions=[
                    "logs:CreateLogGroup",
        ...
        # Least-privilege CloudWatch metrics for the task role: allow publishing
        # custom metrics (PutMetricData is not resource-scopable, so restrict it
        # to the OpenEMR namespace via a condition) instead of CloudWatchFullAccess.
        openemr_fargate_task_definition.task_role.add_to_policy(
            iam.PolicyStatement(
                actions=["cloudwatch:PutMetricData"],
                resources=["*"],
                conditions={
                    "StringEquals": {"cloudwatch:namespace": "OpenEMR"}
                },
            )
        )

        # Allow conenctions to and from both of our EFSs for our Fargate service
        self.openemr_service.connections.allow_from(self.file_system_for_ssl_folder, ec2.Port.tcp(2049))
        self.openemr_service.connections.allow_from(self.file_system_for_sites_folder, ec2.Port.tcp(2049))
        self.openemr_service.connections.allow_to(self.file_system_for_ssl_folder, ec2.Port.tcp(2049))
        self.openemr_service.connections.allow_to(self.file_system_for_sites_folder, ec2.Port.tcp(2049))

        # Allow connections to and from our database and web caching for our fargate service
        self.openemr_service.connections.allow_from(self.db_instance, ec2.Port.tcp(self.mysql_port))
        self.openemr_service.connections.allow_to(self.db_instance, ec2.Port.tcp(self.mysql_port))

        self.openemr_service.connections.allow_from(self.valkey_sec_group, ec2.Port.tcp(self.valkey_port))
        self.openemr_service.connections.allow_to(self.valkey_sec_group, ec2.Port.tcp(self.valkey_port))

        # Allow outbound traffic to SMTP servers operating on port 587
        if self.node.try_get_context("open_smtp_port") == "true":
            self.openemr_service.connections.allow_to_any_ipv4(ec2.Port.tcp(587))

        # Allow communication to the SES interface endpoint if SES is configured
        if self.node.try_get_context("configure_ses") == "true":
            self.openemr_service.connections.allow_to_any_ipv4(ec2.Port.tcp(587))
            self.smtp_interface_endpoint.connections.allow_from(self.openemr_service, ec2.Port.tcp(587))
            self.smtp_interface_endpoint.connections.allow_to(self.openemr_service, ec2.Port.tcp(587))
            self.openemr_service.connections.allow_from(self.smtp_interface_endpoint, ec2.Port.tcp(587))
            self.openemr_service.connections.allow_to(self.smtp_interface_endpoint, ec2.Port.tcp(587))

        # Add CPU and memory utilization based autoscaling
        openemr_scalable_target = (
            self.openemr_service.auto_scale_task_count(
    ...
    def _create_serverless_analytics_environment(self):
        ...
        if self.node.try_get_context("create_serverless_analytics_environment") == "true":
            ...
            unique_id = hashlib.sha256(bytes(f"{self.node.addr}", 'utf-8')).hexdigest().lower()[:18]
            ...
            # This (plus KMS access to analytics_kms_key granted to the
            # export.rds.amazonaws.com service principal above) is what the RDS
            # snapshot-export-to-S3 service role actually needs.
            #
            # NOTE: The previous AmazonRDSDataFullAccess managed policy was
            # removed - it grants rds-data:* (the RDS Data API) which is
            # unrelated to snapshot export to S3 and was an over-grant.
            ...
            # Scope to the specific S3 actions RDS snapshot export requires,
            # instead of the s3:* wildcard action (resource is already limited
            # to the dedicated RDS export bucket).
            rds_export_s3_actions = [
                "s3:PutObject",
                "s3:GetObject",
                "s3:DeleteObject",
                "s3:ListBucket",
            ...
            glue_role.add_to_policy(
                iam.PolicyStatement(
                    ...
                    # Scope to this account's Glue Data Catalog resources
                    # (catalog, databases, tables) instead of a bare wildcard.
                    resources=[
                        f"arn:aws:glue:{self.region}:{self.account}:catalog",
                        f"arn:aws:glue:{self.region}:{self.account}:database/*",
                        f"arn:aws:glue:{self.region}:{self.account}:table/*",
                        f"arn:aws:glue:{self.region}:{self.account}:userDefinedFunction/*",
                    ],
                )
            )
    ...
    def _create_automatic_data_loader(self):
        """Create Lambda function that automatically loads synthetic patient data after deployment.
        
        This provides a "batteries included" demo experience where a single `cdk deploy`
        results in a fully populated OpenEMR system with synthetic patient data.
        """
        
        # Only create if enabled via context
        if self.node.try_get_context("enable_automatic_data_loading") != "true":
            return
        
        # Get private subnet IDs for Lambda VPC configuration
        private_subnets_ids = [ps.subnet_id for ps in self.vpc.private_subnets]
        
        # Create security group for the data loader Lambda
        data_loader_sec_group = ec2.SecurityGroup(
            self,
            "DataLoaderSecurityGroup",
            vpc=self.vpc,
            description="Security group for data loader Lambda",
            allow_all_outbound=True
        )
        
        # Allow Lambda to connect to the database
        self.db_sec_group.add_ingress_rule(
            data_loader_sec_group,
            ec2.Port.tcp(self.mysql_port),
            "Allow data loader Lambda to connect to database"
        )
        
        # Get Orthanc URL from context (optional - if not provided, DICOM loading is skipped)
        orthanc_url = self.node.try_get_context("orthanc_url") or ""
        
        # Get HealthLake sync function name from context (optional)
        healthlake_sync_function = self.node.try_get_context("healthlake_sync_function") or ""
        
        # Create the data loader Lambda function
        data_loader_lambda = _lambda.Function(
            self,
            "DataLoaderLambda",
            runtime=self.lambda_python_runtime,
            ...
            security_groups=[data_loader_sec_group],
            environment={
                'DB_SECRET_ARN': self.db_instance.secret.secret_arn,
                'ORTHANC_URL': orthanc_url,
                'HEALTHLAKE_SYNC_FUNCTION': healthlake_sync_function
            }
        )
        
        # Grant Lambda permission to read database credentials
        self.db_instance.secret.grant_read(data_loader_lambda)
        
        # Create the trigger that runs after deployment
        # Using triggers.TriggerFunction pattern similar to SSL setup
        self.data_loader_trigger = triggers.TriggerFunction(
            self,
            "DataLoaderTrigger",
            runtime=self.lambda_python_runtime,
            code=_lambda.Code.from_asset(
        ...
        # Grant trigger Lambda permission to read database credentials
        self.db_instance.secret.grant_read(self.data_loader_trigger)
        
        # Grant Lambda permission to read Orthanc URL from SSM (for combined deployment)
        self.data_loader_trigger.add_to_role_policy(
            iam.PolicyStatement(
                effect=iam.Effect.ALLOW,
                actions=["ssm:GetParameter"],
                resources=[f"arn:aws:ssm:{self.region}:{self.account}:parameter/openemr/orthanc-url"]
            )
        )
        
        # Grant Lambda permission to invoke HealthLake sync function if configured
        if healthlake_sync_function:
            self.data_loader_trigger.add_to_role_policy(
                iam.PolicyStatement(
                    effect=iam.Effect.ALLOW,
                    actions=["lambda:InvokeFunction"],
                    resources=[f"arn:aws:lambda:{self.region}:{self.account}:function:{healthlake_sync_function}"]
                )
            )
        
        # Ensure data loader runs after database is ready
        self.data_loader_trigger.node.add_dependency(self.db_instance)
        
        # Output information about data loading
        CfnOutput(
            self,
            "DataLoaderStatus",
            value="Automatic data loading enabled - synthetic patients will be loaded on deployment",
            description="Status of automatic data loading feature"
        )

        # Output the admin credentials secret ARN
        CfnOutput(
            self,
            "AdminCredentialsSecret",
            value=self.password.secret_arn,
            description="Secrets Manager ARN for OpenEMR admin credentials"
        )
